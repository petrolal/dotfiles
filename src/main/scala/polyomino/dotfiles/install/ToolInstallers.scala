...
object ToolInstallers:
  ...
  private def installApps(ctx: Context): Either[PolyominoError, Unit] =
    ...
    pm match
      case PackageManager.Pacman =>
        runPkgInstall("sudo", Seq("pacman", "-S", "--needed", "--noconfirm", "waybar", "kitty", "wofi", "swaylock", "gtklock", "swayidle", "grim", "slurp", "wf-recorder", "brightnessctl", "libpulse", "playerctl", "wireplumber", "ttf-jetbrains-mono-nerd", "swaync", "mako", "mpv", "cmake", "ncurses", "neovim", "fastfetch", "jq", "xdotool", "network-manager-applet", "polkit-gnome", "wl-clipboard", "pavucontrol", "python-gobject", "python-cairo", "gtk3", "gtk-layer-shell", "gtk-session-lock", "pam", "zoxide"))
        if isAvailable("yay") then
          ...
          Right(())
      case PackageManager.Dnf =>
        runPkgInstall("sudo", Seq("dnf", "install", "-y", "sway", "waybar", "kitty", "wofi", "swaylock", "swayidle", "grim", "slurp", "wf-recorder", "brightnessctl", "playerctl", "wireplumber", "sway-notification-center", "mako", "mpv", "neovim", "onlyoffice-desktopeditors", "fastfetch", "jq", "xdotool", "network-manager-applet", "polkit-gnome", "wl-clipboard", "pavucontrol", "python3-gobject", "python3-cairo", "gtk3", "gtk-layer-shell", "pam-devel", "zoxide"))
        installSwayfx(ctx)
      case PackageManager.Apt =>
        runPkgInstall("sudo", Seq("apt-get", "install", "-y", "sway", "waybar", "kitty", "wofi", "swaylock", "swayidle", "grim", "slurp", "wf-recorder", "brightnessctl", "playerctl", "wireplumber", "pulseaudio-utils", "fonts-jetbrains-mono", "sway-notification-center", "mako-notifier", "mpv", "python3-gi", "python3-cairo", "gir1.2-gtk-3.0", "gir1.2-gtklayershell-0.1", "libpam0g-dev", "neovim", "fastfetch", "jq", "xdotool", "network-manager-gnome", "policykit-1-gnome", "wl-clipboard", "pavucontrol", "zoxide"))
        installSwayfx(ctx)
      case PackageManager.Brew =>
        runPkgInstall("brew", Seq("install", "fastfetch", "jq", "zoxide"))
      case _ =>
  ...
  def ensureWaylandSession(ctx: Context): Unit =
    ...
    // Only attempt to copy into /usr/share/wayland-sessions/ if we have sudo privileges without prompting
    try
      val canSudo = os.proc("sudo", "-n", "true").call(check = false).exitCode == 0
      if canSudo then
        os.proc("sudo", "-n", "cp", (waylandSessionsDir / "sway.desktop").toString, "/usr/share/wayland-sessions/sway.desktop").call(check = false)
        os.proc("sudo", "-n", "cp", (waylandSessionsDir / "swayfx.desktop").toString, "/usr/share/wayland-sessions/swayfx.desktop").call(check = false)
        ...
        if os.exists(ctx.home / ".local" / "bin" / "sway") then
          os.proc("sudo", "-n", "ln", "-sf", s"${ctx.home}/.local/bin/sway", "/usr/local/bin/sway").call(check = false)
          if os.exists(ctx.home / ".local" / "bin" / "swaymsg") then
            os.proc("sudo", "-n", "ln", "-sf", s"${ctx.home}/.local/bin/swaymsg", "/usr/local/bin/swaymsg").call(check = false)
          if os.exists(ctx.home / ".local" / "bin" / "swaybar") then
            os.proc("sudo", "-n", "ln", "-sf", s"${ctx.home}/.local/bin/swaybar", "/usr/local/bin/swaybar").call(check = false)
          if os.exists(ctx.home / ".local" / "bin" / "swaynag") then
            os.proc("sudo", "-n", "ln", "-sf", s"${ctx.home}/.local/bin/swaynag", "/usr/local/bin/swaynag").call(check = false)
          println(s"  \u001b[32m[OK]\u001b[0m Linked SwayFX binaries to /usr/local/bin")
      else
        println("  \u001b[36m[INFO]\u001b[0m SwayFX session registered locally in ~/.local/share/wayland-sessions/")
    catch
      case _: Exception => ()
